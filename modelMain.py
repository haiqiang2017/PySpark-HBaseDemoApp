...
def loadDataFromHDFS(pathToCSV):
  csv_input = pathToCSV
  hdfs_csv_input = 'hdfs://{0}'.format(csv_input)
  dataframe = spark.read.csv(hdfs_csv_input, inferSchema=True, header=True)
...
if __name__ == "__main__":
  ...
  testDataCtlg = getTestingDataCatalog()
  
  #grabs tables from HBase
  trainingData, nameOfTrainingTableInHBase = loadTableFromHBase(trainDataCtlg)
  testData, nameOfTestingTableInHBase = loadTableFromHBase(testDataCtlg)
  
  trainingData2 = loadDataFromHDFS("/tmp/trainingData.csv")
  ...
  #Uses sample test data to actually use the Model and return a res
  df = useTheModel(actualModel, modelVersion="m1.0.0", dataset=testData, name=nameOfTestingTableInHBase)
  df.show()
    
  
  putTableIntoHBase(df, getModelResultsCatalog())
  
  print("Added " + json.loads(getModelResultsCatalog())["table"]["name"] + " to HBase!")
  
  modelResultsTab, name = loadTableFromHBase(getModelResultsCatalog())
